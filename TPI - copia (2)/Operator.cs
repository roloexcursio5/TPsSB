namespace TPI
{
    internal abstract class Operator
    {
        ...
        // public OperatorBattery2 Battery { get; set; } //mAh miliAmperios, 1000mAh = 1 hour use
        ...
        protected bool TryTransferElementTo(OperatorElement operatorElement, Operator operatorTheOther, OperatorElement operatorElement2)
        {
            string message = "";

            //if (!Map.CheckSameLocation(Location, operatorTheOther.Location))
            //    message += "Los operadores no están en el mismo lugar\n";
            if (operatorElement.ElementToGive() <= 0)
                message += Messages.NotEnoughElementToGiveMessage(operatorElement);
            if (operatorElement2.ElementToReceive() == 0)
                message += Messages.ElementToReceiveFullMessage(operatorElement);

            if (message != "")
                message += Messages.ElementToTransferImposibilityMessage(operatorElement);
        ...
        protected int ElementTransferAmount(OperatorElement operatorElement, OperatorElement operatorElement2)
        {
            int Give = operatorElement.ElementToGive();
            int Receive = operatorElement2.ElementToReceive();

            return Give > Receive ? Receive : Give;
        }

        public void ElementTransferTo(OperatorElement operatorElement, Operator operatorTheOther, OperatorElement operatorElement2)
        {
            if (TryTransferElementTo(operatorElement, operatorTheOther, operatorElement2))
            {
                int batteryTransfered = ElementTransferAmount(operatorElement, operatorElement2);
                ...
                operatorTheOther.OperatorDamages.UnionWith(new Damages().DamageRandomCreator());
            }
        }
        /// <param name="operatorTheOther"></param>
        /// <returns></returns>
